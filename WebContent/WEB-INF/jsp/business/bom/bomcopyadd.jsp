...
<div id="main">
	...
	<form:form modelAttribute="bomForm" method="POST"
		...
		<form:hidden path="bomPlan.sourcebomid" value="${bomPlan.bomPlan.bomid}" />		
		
		<fieldset>
			<legend> BOM方案　-　复制新建</legend>
		...
		<c:if test="${fn:length(bomDetail) > 0}" >
			...
			<c:forEach var="detail" items="${bomDetail}" varStatus='status' >		
				...
				<script type="text/javascript">
					...
					productCost = productCost + currencyToFloat(cost);	
					//alert("total111:"+productCost)
					counter++;
				</script>
				<tr>
					<td></td>
					...
					<td><span>${detail.totalPrice}</span>
						<form:hidden path="bomDetailLines[${status.index}].totalprice" value="${detail.totalPrice}"/></td>
					<td><span>${detail.lastPrice}</span></td>
					<td>${detail.price}
						<form:hidden path="bomDetailLines[${status.index}].sourceprice" value="${detail.price}"/></td>
