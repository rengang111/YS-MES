...
<script type="text/javascript">
	...
	function ajax() {
		var table = $('#TMakeDocument').dataTable();
		if(table) {
			table.fnDestroy();
		}
	
		var t = $('#TMakeDocument').DataTable({
				"paging": true,
				...
				"retrieve" : true,
				"sAjaxSource" : "${ctx}/business/makedocument?methodtype=search",
	...
	function initEvent(){
		...
		$('#TMakeDocument').DataTable().on('click', 'tr', function() {
			
			if ( $(this).hasClass('selected') ) {
	            $(this).removeClass('selected');
	        }
	        else {
	        	$('#TMakeDocument').DataTable().$('tr.selected').removeClass('selected');
	            $(this).addClass('selected');
	        }
		});
		
		/*
		$('#TMakeDocument').DataTable().on('dblclick', 'tr', function() {

			var d = $('#TMakeDocument').DataTable().row(this).data();
	...
	function doCreate() {
		...
		var url = "${ctx}/business/makedocument?methodtype=addinit";
	...
	function doUpdate(key) {
		var str = '';
		var isFirstRow = true;
		var url = "${ctx}/business/makedocument?methodtype=updateinit&key=" + key;
		...
		if (str != '') {
			if(confirm("确定要删除数据吗？")) {
				jQuery.ajax({
					...
					dataType : 'json',
					data : str,
					url : "${ctx}/business/makedocument?methodtype=delete",
					success : function(data) {
						reload();
						//alert(data.message);
	...
	function reload() {
		...
		$('#TMakeDocument').DataTable().ajax.reload(null,false);
...
<div id="container">
		...
		<div id="main">
			...
			<div class="list">
				...
				<div id="TMakeDocument_wrapper" class="dataTables_wrapper">
					...
					<div id="clear"></div>
					<table aria-describedby="TMakeDocument_info" style="width: 100%;" id="TMakeDocument" class="display dataTable" cellspacing="0">
						<thead>
