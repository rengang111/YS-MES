...
<script type="text/javascript">
	...
	function addSubCodeTr(activeSubCode, subCode, subName, id, isInit){
		...
		if (activeSubCode == '' || activeSubCode == id) {
			...
			trHtml += "<input name='mouldSubs[" + i + "].subcodeid' id='mouldSubs[" + i + "].subcodeid' type='hidden' value='" + id + "'/>";
			trHtml += "<input name='mouldSubs[" + i + "].subcode' id='mouldSubs[" + i + "].subcode' type='text'  style='width:20px' value='" + subCode + "'/>";
			trHtml += "</td>";
			if (activeSubCode != '') {
			...
			trHtml += "<textarea name='mouldSubs[" + i + "].name' id='mouldSubs[" + i + "].name' cols=60 rows=1	>" + subName +  "</textarea>";
			trHtml += "</td>";
			trHtml += "</tr>";			
			$('#subidTab tr:last').after(trHtml);

			$('#mouldSubs\\[' + i + '\\]\\.subcode').rules('add', { noDuplicateSubcode: true});
			$('#mouldSubs\\[' + i + '\\]\\.subcode').rules('add', { maxlength: 2 });
	...
	function initEvent(){
		...
		<c:forEach items="${DisplayData.mouldSubDatas}" var="item">
			addSubCodeTr('${DisplayData.activeSubCode}', '${item.subCode}', '${item.name}', '${item.id}', false);
	...
	function doEdit() {
		if (validator.form() && $('#subCodeCount').val() != '') {
			$.ajax({
				async:false,
				type : "POST",
				url : "${ctx}/business/mouldregisterequipment?methodtype=updateequipment",
				...
				success : function(d) {
					if (d.rtnCd != "000") {
						...
						var url = '${ctx}/business/mouldregisterequipment?methodtype=updateinit&key=' + $('#keyBackup').val() + "&activeSubCode=" + $('#activeSubCode').val();
						location.href = url;
...
<div id="container">
		...
		<div id="main">
			<form:form modelAttribute="dataModels" id="mouldBaseInfo" style='padding: 0px; margin: 10px;' >
				...
				<input type=hidden id="activeSubCode" name="activeSubCode" value=""/>
				...
				<div style="margin: -70px 0px 0px 0px;width:65%">
					...
					<button type="button" id="edit" class="DTTT_button" style="height:25px;margin:-15px 10px 0px 0px;float:right;" onClick="doEdit();">保存</button>
								...
								<table width='700px'>
									<tr>
										...
										<td class="td-center" >编码解释
											...
											id="createSubid" class="DTTT_button" onClick="addSubCodeTr('', '', '', '', false);">新建</button>
										</td>
									</tr>
								</table>
							</td>
