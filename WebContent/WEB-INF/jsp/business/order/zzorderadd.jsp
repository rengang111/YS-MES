...
function ajax() {
	...
	t.on('change', 'tr td:nth-child(4)',function() {
		...
		*/
		...
		// alert(materialid)
        if(materialid == "")
        	$omaterial.removeClass("bgwhite").addClass("required");
...
function doSubmitReturn(){
	...
	$('#ZZOrder').attr("action", "${ctx}/business/zzorder?methodtype=insert");
	$('#ZZOrder').submit();
}
...
<form:form modelAttribute="ZZOrder" method="POST" 
	...
	<form:hidden path="order.recordid" value=""/>
	...
	<table id="baseinfo" class="form" width="100%">		
		<tr>
			...
			<td style="width: 120px;">
				<form:input path="order.orderid" class="required read-only" value="${ysCode }"/></td>
	...
	<table id="example" class="display" width="100%">	
		...
		<tbody>
			<c:forEach var="i" begin="0" end="4" step="1">		
				<tr>
					<td></td>
					<td><form:input path="orderDetailLines[${i}].materialid" class="attributeList1"/></td>								
					<td><span></span></td>
					<td><form:input path="orderDetailLines[${i}].quantity" class="num mini" /></td>							
					<td><span></span><form:hidden path="orderDetailLines[${i}].price"  /></td>
					<td><span></span>
						<form:hidden path="orderDetailLines[${i}].totalprice"  /></td>				
					<td><input type="hidden" id="orderDetailLines[${i}].description"  /></td>
				</tr>					
		...
		<tfoot>
			<tr>
				...
				<td class="td-right" >
					...
					<form:hidden path="order.totalprice"  /></td>
				<td></td>
			</tr>
		</tfoot>
	</table>
