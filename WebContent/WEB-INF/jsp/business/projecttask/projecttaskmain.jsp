...
<script type="text/javascript">
	...
	function ajax() {
		var table = $('#TProjectTask').dataTable();
		if(table) {
			table.fnDestroy();
		}
	
		var t = $('#TProjectTask').DataTable({
				"paging": true,
				...
				"retrieve" : true,
				"sAjaxSource" : "${ctx}/business/projecttask?methodtype=search",
				...
				"columns": [
							...
							{"data": "referPrototype", "defaultContent" : '',"className" : 'td-center'},
	...
	function initEvent(){
		...
		$('#TProjectTask').DataTable().on('click', 'tr', function() {
			
			if ( $(this).hasClass('selected') ) {
	            $(this).removeClass('selected');
	        }
	        else {
	        	$('#TProjectTask').DataTable().$('tr.selected').removeClass('selected');
	            $(this).addClass('selected');
	        }
		});
		
		/*
		$('#TProjectTask').DataTable().on('dblclick', 'tr', function() {

			var d = $('#TProjectTask').DataTable().row(this).data();
		...
		if (str != '') {
			if(confirm("确定要删除数据吗？")) {
				jQuery.ajax({
					...
					dataType : 'json',
					data : str,
					url : "${ctx}/business/projecttask?methodtype=delete",
					success : function(data) {
						reload();
						//alert(data.message);
	...
	function reload() {
		...
		$('#TProjectTask').DataTable().ajax.reload(null,false);
...
<div id="container">
		...
		<div id="main">
			...
			<div class="list">
				...
				<div id="TProjectTask_wrapper" class="dataTables_wrapper">
					<div id="DTTT_container" align="right" style="height:40px">
						<a aria-controls="TProjectTask" tabindex="0" id="ToolTables_TProjectTask_0" class="DTTT_button DTTT_button_text" onClick="doCreate();"><span>新建</span></a>
						<a aria-controls="TProjectTask" tabindex="0" id="ToolTables_TProjectTask_1" class="DTTT_button DTTT_button_text" onClick="doDelete();"><span>删除</span></a>
					</div>
					<div id="clear"></div>
					<table aria-describedby="TProjectTask_info" style="width: 100%;" id="TProjectTask" class="display dataTable" cellspacing="0">
						<thead>
							...
							<tr class="selected">
								...
								<th colspan="1" rowspan="1" style="width: 120px;" aria-label="参考原型" class="dt-middle sorting_disabled">参考原型</th>
