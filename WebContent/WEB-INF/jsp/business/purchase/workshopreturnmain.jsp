...
<script type="text/javascript">
	...
	function ajax(pageFlg,status) {
		...
		var t = $('#TMaterial').DataTable({
			...
			"retrieve" : true,
			"sAjaxSource" : "${ctx}/business/contract?methodtype=searchWorkshopReturn&keyBackup="+pageFlg+"&status="+status,
			...
			"columnDefs":[
	    		...
	    		{"targets":3,"render":function(data, type, row){
	    			...
	    			return "<a href=\"###\" onClick=\"doShowControct('" + row["contractId"] + "')\">"+row["materialId"]+"</a>";			    			
	    		}},
	    		{"targets":4,"render":function(data, type, row){
	    			var name = row["materialName"];				    			
	    			if(name != null) name = jQuery.fixedWidth(name,45);
	    			return name;
	    		}},
	    		{"targets":7,"render":function(data, type, row){
	    			var quantity = row["returnGoods"];
	    			if(quantity < 1){

		    			return quantity;	
	    			}else{

		    			return "<a href=\"###\" onClick=\"doShowDetail('" + row["YSId"] + "','" + row["contractId"] + "')\">"+row["returnGoods"]+"</a>";	
	    			}		    			
	    		}}
         	] 
		});
	...
	function doShowDetail(YSId,contractId) {

		var url = '${ctx}/business/contract?methodtype=workshopRentunDetailView&YSId=' + YSId+'&contractId='+contractId;
	...
	function doShowControct(contractId) {
		...
		var url = '${ctx}/business/contract?methodtype=createWorkshopRentunInit&contractId=' + contractId;
		location.href = url;
	}
		...
		<table id="TMaterial" class="display dataTable" >
			<thead>						
				<tr>
					...
					<th style="width: 120px;"  class="dt-middle ">合同编号</th>
