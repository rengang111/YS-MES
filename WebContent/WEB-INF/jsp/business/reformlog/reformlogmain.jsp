...
<script type="text/javascript">
	...
	function ajax() {
		var table = $('#TReformLog').dataTable();
		if(table) {
			table.fnDestroy();
		}
	
		var t = $('#TReformLog').DataTable({
				"paging": true,
				...
				"retrieve" : true,
				"sAjaxSource" : "${ctx}/business/reformlog?methodtype=search",
	...
	function initEvent(){
		...
		$('#TReformLog').DataTable().on('click', 'tr', function() {
			
			if ( $(this).hasClass('selected') ) {
	            $(this).removeClass('selected');
	        }
	        else {
	        	$('#TReformLog').DataTable().$('tr.selected').removeClass('selected');
	            $(this).addClass('selected');
	        }
		});
		
		/*
		$('#TReformLog').DataTable().on('dblclick', 'tr', function() {

			var d = $('#TReformLog').DataTable().row(this).data();
	...
	function doCreate() {
		...
		var url = "${ctx}/business/reformlog?methodtype=addinit";
	...
	function doUpdate(key) {
		var str = '';
		var isFirstRow = true;
		var url = "${ctx}/business/reformlog?methodtype=updateinit&key=" + key;
		...
		if (str != '') {
			if(confirm("确定要删除数据吗？")) {
				jQuery.ajax({
					...
					dataType : 'json',
					data : str,
					url : "${ctx}/business/reformlog?methodtype=delete",
					success : function(data) {
						reload();
						//alert(data.message);
	...
	function reload() {
		...
		$('#TReformLog').DataTable().ajax.reload(null,false);
...
<div id="container">
		...
		<div id="main">
			...
			<div class="list">
				...
				<div id="TReformLog_wrapper" class="dataTables_wrapper">
					...
					<div id="clear"></div>
					<table aria-describedby="TReformLog_info" style="width: 100%;" id="TReformLog" class="display dataTable" cellspacing="0">
						<thead>
