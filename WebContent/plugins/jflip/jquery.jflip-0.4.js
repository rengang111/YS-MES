...
(function($180){
    ...
    var Flip = function(canvas,width,height,images,opts) {
      //private vars
      opts = $180.extend({background:"green",cornersTop:true,scale:"noresize"},opts);
      var obj = this,
      el = canvas.prev(),
      index = 0,
      init = false,
      ...
      ctx2 = $180.browser.msie?null:canvas2[0].getContext("2d"),
      canvas = $180.browser.msie?$180(G_vmlCanvasManager.initElement(canvas[0])):canvas,
      ...
      var
        ...
        off = $180.browser.msie?canvas.offset():null,
        onCorner = false,
      ...
      c.mousemove(function(e){
        ...
        */
        ...
        return false;
    	  ...
    	  offsetX1 = Math.abs(e.clientX-off.left - width/2);
    	  offsetY1 = Math.abs(e.clientY-off.top - height/2);
    	  
    	  //alert(offsetX1);
    	  //alert(offsetY1);   
    	  
    	  if ((offsetX1<=100)&&(offsetY1<=100)){
    		  
    		  filename = images[0].src;
    		  //alert(filename);
    		  pos = filename.indexOf('/YS');
    		  //alert(filename.substr(pos));
    		  
    		  showRealPhoto(filename.substr(pos));
    	  }    		  
      ...
      draw = function() {
        ...
        var img = images[0], r = $180(img).data("flip.scale");
        if($180.browser.msie) {
        ...
        if(mY && mX!=width) {
          ...
          var c;
          if(!($180.browser.mozilla && parseFloat($180.browser.version)<1.9)) {
            c = ctx;
          } else {
            c = ctx2;
          ...
          */
          ...
          if(!$180.browser.safari && !$180.browser.opera)
           ctx.restore();          

          var img = sideLeft?images[images.length-1]:images[1];
          r = $180(img).data("flip.scale");
          if($180.browser.msie) {
    ...
    $180.fn.jFlip = function(width,height,opts){
    
      return this.each(function() {
        $180(this).wrap("<div class='flip_gallery'>");
        var images = $180(this).find("img");
        //cannot hide because explorer does not give the image dimensions if hidden
        var canvas = $180(document.createElement("canvas")).attr({width:width,height:height}).css({margin:0,width:width+"px",height:height+"px"})
        $180(this).css({position:"absolute",left:"-9000px",top:"-9000px"}).after(canvas);
        new Flip($180(this).next(),width || 300,height || 300,images,opts);
      });
    };
