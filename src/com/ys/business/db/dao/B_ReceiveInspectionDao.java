...
public class B_ReceiveInspectionDao extends BaseAbstractDao
{
	...
	public String Create(Object beanDataTmp) throws Exception
	{
		...
		{
			throw new Exception("INSERT INTO B_ReceiveInspection( recordid,ysid,contractid,arrivalid,supplierid,materialid,quantity,report,memo,lossandcisposal,arrivedate,delaydays,inspecttime,waitdays,deptguid,createtime,createperson,createunitid,modifytime,modifyperson,deleteflag,formid)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)��"+ e.toString());
		}
		finally
		{
	...
	public String getInsertSQL()
	{
		...
		try
		{
			bufSQL.append("INSERT INTO B_ReceiveInspection( recordid,ysid,contractid,arrivalid,supplierid,materialid,quantity,report,memo,lossandcisposal,arrivedate,delaydays,inspecttime,waitdays,deptguid,createtime,createperson,createunitid,modifytime,modifyperson,deleteflag,formid)VALUES(");
	...
	public void Store()  throws Exception
	{
		...
		{
			connection = getConnection();
			statement = connection.prepareStatement("UPDATE B_ReceiveInspection SET recordid= ? , ysid= ? , contractid= ? , arrivalid= ? , supplierid= ? , materialid= ? , quantity= ? , report= ? , memo= ? , lossandcisposal= ? , arrivedate= ? , delaydays= ? , inspecttime= ? , waitdays= ? , deptguid= ? , createtime= ? , createperson= ? , createunitid= ? , modifytime= ? , modifyperson= ? , deleteflag= ? , formid=? WHERE  recordid  = ?");
			statement.setString( 1,beanData.getRecordid());			statement.setString( 2,beanData.getYsid());			statement.setString( 3,beanData.getContractid());			statement.setString( 4,beanData.getArrivalid());			statement.setString( 5,beanData.getSupplierid());			statement.setString( 6,beanData.getMaterialid());			statement.setString( 7,beanData.getQuantity());			statement.setString( 8,beanData.getReport());			statement.setString( 9,beanData.getMemo());			statement.setString( 10,beanData.getLossandcisposal());			if (beanData.getArrivedate()==null || beanData.getArrivedate().trim().equals(""))			{				statement.setNull( 11, Types.DATE);			}			else			{				statement.setString( 11, beanData.getArrivedate());			}			statement.setString( 12,beanData.getDelaydays());			statement.setString( 13,beanData.getInspecttime());			statement.setString( 14,beanData.getWaitdays());			statement.setString( 15,beanData.getDeptguid());			statement.setString( 16,beanData.getCreatetime());			statement.setString( 17,beanData.getCreateperson());			statement.setString( 18,beanData.getCreateunitid());			statement.setString( 19,beanData.getModifytime());			statement.setString( 20,beanData.getModifyperson());			statement.setString( 21,beanData.getDeleteflag());			statement.setString( 22,beanData.getFormid());
		...
		{
			throw new Exception("UPDATE B_ReceiveInspection SET recordid= ? , ysid= ? , contractid= ? , arrivalid= ? , supplierid= ? , materialid= ? , quantity= ? , report= ? , memo= ? , lossandcisposal= ? , arrivedate= ? , delaydays= ? , inspecttime= ? , waitdays= ? , deptguid= ? , createtime= ? , createperson= ? , createunitid= ? , modifytime= ? , modifyperson= ? , deleteflag= ? , formid=? WHERE  recordid  = ?"+ e.toString());
		}
		finally
		{
			closeConnection(connection, statement);
