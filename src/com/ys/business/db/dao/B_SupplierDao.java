...
public class B_SupplierDao extends BaseAbstractDao
{
	...
	public Object FindByPrimaryKey(Object beanDataTmp)  throws Exception
	{
		...
		{
			connection = getConnection();
			statement = connection.prepareStatement("SELECT recordid,supplierid,shortname,suppliername,parentid,subid,categoryid,categorydes,paymentterm,country,province,city,address,zipcode,type,normaldelivery,maxdelivery,issues,deptguid,createtime,createperson,createunitid,modifytime,modifyperson,deleteflag,formid FROM B_Supplier WHERE  recordid =?");
			statement.setString( 1,beanData.getRecordid());
		...
		{
			throw new Exception("Error executing SQL SELECT recordid,supplierid,shortname,suppliername,parentid,subid,categoryid,categorydes,paymentterm,country,province,city,address,zipcode,type,normaldelivery,maxdelivery,issues,deptguid,createtime,createperson,createunitid,modifytime,modifyperson,deleteflag,formid FROM B_Supplier  WHERE  "+e.toString());
		}
		finally
		{
			closeConnection(connection, statement);
	...
	public java.util.Vector Find(String astr_Where)  throws Exception
	{
		...
		try
		{
			...
			if(!str_Where.equals(""))
				str_Where=" WHERE " + str_Where ; 
			statement = connection.prepareStatement("SELECT recordid,supplierid,shortname,suppliername,parentid,subid,categoryid,categorydes,paymentterm,country,province,city,address,zipcode,type,normaldelivery,maxdelivery,issues,deptguid,createtime,createperson,createunitid,modifytime,modifyperson,deleteflag,formid FROM B_Supplier"+str_Where);
			ResultSet resultSet = statement.executeQuery();
		...
		{
			throw new Exception("Error executing SQL SELECT recordid,supplierid,shortname,suppliername,parentid,subid,categoryid,categorydes,paymentterm,country,province,city,address,zipcode,type,normaldelivery,maxdelivery,issues,deptguid,createtime,createperson,createunitid,modifytime,modifyperson,deleteflag,formid FROM B_Supplier" + astr_Where +e.toString());
		}
		finally
		{
			closeConnection(connection, statement);
	...
	public void Store()  throws Exception
	{
		...
		{
			throw new Exception("UPDATE B_Supplier SET recordid= ? , supplierid= ? , shortname= ? , suppliername= ? , parentid= ? , subid= ? , categoryid= ? , categorydes= ? , paymentterm= ? , country= ? , province= ? , city= ? , address= ? , zipcode= ? , type= ? , normaldelivery= ? , maxdelivery= ? , issues= ? , deptguid= ? , createtime= ? , createperson= ? , createunitid= ? , modifytime= ? , modifyperson= ? , deleteflag= ? , formid=? WHERE  recordid  = ?"+ e.toString());
		}
		finally
		{
			closeConnection(connection, statement);
